...
const CommunalProjectsManagementPage = () => {
  ...
  const fetchCommunalData = async () => {
    try {
      ...
      setMessage({ type: 'error', text: 'Failed to load communal projects data' })
    } finally {
      setFetching(false)
    }
  }
  ...
  const saveHeroImage = async () => {
    ...
    try {
      ...
      formData.append('alt', 'Communal Projects Hero')

      const res = await fetch(`${API_URL}/communal-projects/hero-image`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      <AdminLayout>
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Loading communal projects data...</p>
          </div>
        </div>
      </AdminLayout>
    )
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-gray-800">Communal Projects Management</h1>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          {communalData?.galleryImages && communalData.galleryImages.length > 0 ? (
...
export default CommunalProjectsManagementPage
