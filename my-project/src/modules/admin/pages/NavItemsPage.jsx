...
const NavItemsPage = ({ group, title, description, allowAdd = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const effectiveKey = slugify(formData.name)
                ...
                {loading ? (
                  ...
                  items.map((item) => (
                    <tr key={item._id || item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-600">{item.isActive ? 'Yes' : 'No'}</td>
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl mx-4">
              <div className="flex items-center justify-between border-b px-6 py-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600">Update the hover card image, label, and order.</p>
                </div>
