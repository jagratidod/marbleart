...
const HomeDecorManagementPage = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            if (result.success) {
                ...
                if (!selectedCategory && result.data.length > 0 && result.data[0].categories.length > 0) {
                    setSelectedCategory(result.data[0].categories[0])
                }
            }
        } catch (error) {
    ...
    const fetchProducts = async (categoryId) => {
        ...
        try {
            const res = await fetch(`${API_URL}/home-decor/products/${categoryId}`)
            const result = await res.json()
    ...
    const handleUploadHero = async (e) => {
        ...
        reader.onloadend = async () => {
            ...
            try {
                ...
                const res = await fetch(`${API_URL}/home-decor/categories`, {
                    method: 'POST',
    ...
    const handleUpsertProduct = async (productData) => {
        try {
            setLoadingProducts(true)
            const res = await fetch(`${API_URL}/home-decor/products`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    ...
    const handleDeleteProduct = async (id) => {
        ...
        try {
            const res = await fetch(`${API_URL}/home-decor/products/${id}`, {
                method: 'DELETE',
        ...
        <AdminLayout>
            <div className="p-4 md:p-8 space-y-8 min-h-screen">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-800">Home Decor Management</h1>
                        <p className="text-gray-500">Manage categories, products and home decor items.</p>
                    </div>
                </div>
                            ...
                            {selectedCategory && (
                                <div className="space-y-6 animate-fadeIn">
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-center">
                                            <h3 className="text-xl font-bold text-gray-800">Products in {selectedCategory.name} ({products.length})</h3>
                                            <button
...
const ProductDialog = ({ product, onClose, onSave, loading }) => {
    const [formData, setFormData] = useState(product || {
        name: '', sku: '', price: '', material: '', size: '', dimensions: '', description: '', images: []
    })
...
export default HomeDecorManagementPage
